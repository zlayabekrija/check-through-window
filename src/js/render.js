    ...
    fulldata.classList.add('w-75', 'm-auto', 'd-flex', 'justify-content-center', 'info', "master");
    const extract = (data, loader) => {
    ...
    const modal = (statement) => {
      const box = document.createElement("div")
      const parentText = document.createElement("div")
      const parentClosing = document.createElement("div")
      const text = document.createElement("p")
      const closingText = document.createElement("p")
      const X = document.createElement("p")
      X.innerText = "X"
      text.innerText = "An error occured. Maybe the location you are looking for doesn't exist. Please try again"
      closingText.innerText = "Please close this modal by clicking on X or pressing ESC button."
      box.classList.add("main-frame")
      X.classList.add("closing-btn")
      parentText.append(text)
      parentClosing.append(closingText)
      box.append(parentText, parentClosing, X)
      document.querySelector("#weather").prepend(box)
      if (!statement) {
